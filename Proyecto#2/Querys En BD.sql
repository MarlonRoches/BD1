...
values ('/*****	Check	*****/	
select * from [calculo de horas extras]
...
select CONEMP.* from PROYECTOS PR
join ALBAÑIL AL on AL.IDAlbañil = PR.FK_IDCapataz
join CONTRATO_EMPLEADO CONEMP on CONEMP.FK_IDEmpleado = AL.IDAlbañil')
insert into Querys (Query)
values ('/*****	Check	*****/	
select top 1 Arq.[ID Arquitecto] ,count(Pry.CodigoProyecto) as[Proyectos Realizados] from PROVEEDORES Prov /*,Prov.Nombre, Pry.CodigoProyecto  ,   */
...
')
insert into Querys (Query)
values ('select ConEmp.FK_IDEmpleado as[Id Arquitecto],
...
insert into Querys (Query)
values ('/*****	Check	*****/	
select Top 1 Mqn.Nombre as [Tipo de Maquinaria],count(*) as [Usos en Proyecto] from MAQUINARIA Mqn
